...
class AgentManager:
    ...
    def get_final_response(self, search_results, context_dict):
        ...
        system_prompt = (
            ...
            "2. CITATIONS: When citing sources in the text, use markdown links: [Filename](URI).\n"
            "   - The visible text (Filename) should be ONLY the clean filename (e.g., 'Report.pdf').\n"
            "   - The URI should be the full path/link exactly as provided in the context.\n"
            "   - Example: [ProjectSheet.pdf](C:\\Users\\imain\\Adept Technologies Ltd\\30. Cloud & Business Automation - Documents\\folder\\ProjectSheet.pdf)\n"
            "3. REFERENCES: At the end, list unique sources under a 'References' header.\n"
            "   - Format each as a bullet point with markdown link: • [Filename](URI)\n"
            "   - Filename = clean filename only (e.g., 'OldMutual.pptx')\n"
            "   - URI = full path/link from context\n"
            "   - Example: • [2025-CompanyProfile.pdf](C:\\path\\to\\file.pdf)\n"
        )
